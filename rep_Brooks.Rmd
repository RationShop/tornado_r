...
Create a uniform lat-lon grid of 0.5 degrees spanning the mainland US. Brooks et al used a uniform grid of 80-km. But for ease of computation, visualization and storage a 0.5 degree grid is used. The results from this approach will be compared with those from Brooks et al, and if needed, a different spatial grid would be adopted.

```{r}
lat_seq <- seq(24, 50, 0.5)
lon_seq <- seq(-125, -65, 0.5)
ll_grid <- expand.grid(lat = lat_seq, lon = lon_seq, KEEP.OUT.ATTRS = FALSE)
```
...
```{r}
torn_8099 <- torn_8099[, c("SLAT", "SLON", "ELAT", "ELON", "LENGTH", "id")]
str(torn_8099)
```
