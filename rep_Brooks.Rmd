...
```{r}
wts_vec <- rep(0, length(lat_seq) * length(lon_seq))
dist_mat <- matrix(0, nrow = length(lat_seq), ncol = length(lon_seq), byrow = TRUE)
...
for (eachRow in 1:length(lat_seq)) {
  for (eachCol in 1:length(lon_seq)) {
    dist_vec <- Fn_Haversine_Distance(lat_seq[eachRow], lon_seq[eachCol], ll_df$lat, ll_df$lon)
    for (eachDoy in c(1:366)) {
      wts_vec[eachCol + (eachRow - 1) * length(lon_seq)] <- 0
    }
  }
}
```
