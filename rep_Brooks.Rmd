...
ae_proj <- "+proj=aeqd +lat_0=40 +lon_0=-95 +units=m"
```
...
# usa_rast <- raster(matrix(1:(aeX*aeY), nrow = aeY), 
...
Function to compute the euclidean distance between 2 points on a equidistant grid.
...
for (eachRow in 1:aeY) {
  for (eachCol in 1:aeX) {
    ...
    dist_mat <- (aegrid_res / 1000) * Fn_Compute_Distance(yindx_mat, xindx_mat, 
                                                          eachRow, eachCol) 
    # spatial smoothing, equation 2                   
    for (eachDoy in c(180:180)) {
      space_smooth[eachRow, eachCol, eachDoy] <- sum(space_raw[, , eachDoy] * exp(-0.5 * (dist_mat / sigma_x)^2)) / (2 * pi * (sigma_x^2))                                         
    }
  }
}
...
```{r}
ann_avg <- space_smooth[,,180]
ann_avg <- apply(ann_avg, c(1,2), FUN = sum)
summary(ann_avg)
```
