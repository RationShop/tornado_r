...
Create a uniform grid of 80-km spanning the mainland US, consistent with Brooks et al.
...
# lat-lon and equidistant projection info
ll_proj <- "+proj=longlat +datum=WGS84"
new_proj <- "+proj=aeqd +lat_0=40 +lon_0=-95 +units=m"

new_ref_locs <- SpatialPointsDataFrame(coords = ll_df[, c("lon", "lat")], 
                                       data = ll_df, 
                                       proj = CRS(ll_proj))

# project lat-lons to aeqd
new_ref_locs <- spTransform(new_ref_locs, CRS(new_proj))

# create raster in aeqd
cell_res <- 80000 # raster resolution in meters 
new_bounds <- apply(new_ref_locs@coords, 2, range)
...
Fn_Identify_Grids <- function(torn_data) {
  ...
  if (!(begLat == 0 | begLon == 0 | sum(any(is.na(torn_data))) > 0)) {
    ...
    sloc <- SpatialPointsDataFrame(coords = torn_data[, c("SLON", "SLAT")],
                               data = torn_data, 
                               proj = CRS(ll_proj))
    sloc <- spTransform(sloc, CRS(new_proj))
    sloc <- sloc@coords
    
    out_list <- c(out_list, 
                  paste0(findInterval(sloc[2], y_breaks), "_", findInterval(sloc[1], x_breaks)))
    
    if (endLat != 0 & endLon != 0) {
      # indices of ending lat-lon, corresp to "ELON" & "ELAT"
      eloc <- SpatialPointsDataFrame(coords = torn_data[, c("ELON", "ELAT")],
                               data = torn_data, 
                               proj = CRS(ll_proj))
      eloc <- spTransform(eloc, CRS(new_proj))
      eloc <- eloc@coords
      
      out_list <- c(out_list, 
                  paste0(findInterval(eloc[2], y_breaks), "_", findInterval(eloc[1], x_breaks)))
      ...
      bet_y <- findInterval(bet_y, y_breaks)
      bet_x <- findInterval(bet_x, x_breaks)
...
```{r}
space_raw <- matrix(data = 0, nrow = length(y_breaks) * length(x_breaks), ncol = 366)
for (eachDoy in c(1:366)) {
  ...
  if (sum(any(is.na(doy_grids))) == 0) {
    ...
    ll_index <- ll_index[, 2] + (ll_index[, 1] - 1) * length(x_breaks)
    
    # populate the parent matrix, when there is an occurence, with the smoothed
    # temporal frequency
    space_raw[c(ll_index), eachDoy] <- unique(torn_doy[, "freq_smooth15"])
  }
}
```

Function to compute distance between 2 points on a equidistant grid.
...
for (eachRow in 1:length(lat_seq)) {
  for (eachCol in 1:length(lon_seq)) {
    dist_mat[eachCol + (eachRow - 1) * length(lon_seq), ] <- Fn_Compute_Distance(lat_seq[eachRow], lon_seq[eachCol], ll_df$lat, ll_df$lon)
  }
}
