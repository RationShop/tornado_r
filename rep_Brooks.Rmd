...
Fn_Draw_Spatial_Freq <- function(doy, gfx_name, smoothed = TRUE, cumulative = FALSE) {
  ...
  png(paste0(gfx_name, "_", doy, ".png"), width = ncol(doy_rast)*10, height = nrow(doy_rast)*10)
  if (cumulative) {
    leg_breaks <- seq(0.25, 1.25, 0.25)
    plot(doy_rast, breaks = leg_breaks, col = rev(rainbow(4)), lab.breaks = leg_breaks, zlim = range(leg_breaks), axes = FALSE, main = "Number of Tornadoes Per Year, 1980-99")
  } else {
...
Fn_Draw_Spatial_Freq(366, "doy_cumul_smooth", smoothed = TRUE, cumulative = TRUE)
